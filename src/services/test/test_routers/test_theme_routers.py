...
class TestThemeRouters:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (status.HTTP_200_OK, Data.user_id, list()),
            (status.HTTP_404_NOT_FOUND, "qqqqqqqq", list())
        ]
    )
    ...
    @pytest.mark.parametrize(
        "code, theme_id, to_update",
        [
            (status.HTTP_200_OK, None, Data.to_update),
            (status.HTTP_404_NOT_FOUND, Data.non_exist_id, Data.to_update),
            (status.HTTP_400_BAD_REQUEST, "123456", Data.to_update)
        ]
    )
    async def test_update_theme(self, ac: AsyncClient, code, theme_id, to_update):
        if code == status.HTTP_200_OK:
            theme_id = pytest.theme_cash.theme_id_1
