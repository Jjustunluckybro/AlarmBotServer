...
async def get_user(r: Request, user_telegram_id: str, db: IDataBase = Depends(get_db)) -> UserModel:
    """"""
...
async def create_user(r: Request, user: UserModel, db: IDataBase = Depends(get_db)) -> str:
...
async def update_username(r: Request, user_telegram_id: str, new_name: str, db: IDataBase = Depends(get_db)) -> int:
    try:
...
async def delete_user(r: Request, user_telegram_id: str, db: IDataBase = Depends(get_db)) -> int:
    try:
